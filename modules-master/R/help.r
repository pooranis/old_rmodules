parse_documentation = function (module) {
    ...
    parsed = list(env = module,
                  blocks = roxygen2501:::parse_file(module_path, module))
    roclet = roxygen2501::rd_roclet()
    rdfiles = roxygen2501:::roc_process(roclet, parsed, dirname(module_path))
    ...
    formatted = lapply(unlist(docs, recursive = FALSE, use.names = FALSE),
                       roxygen2501:::format.rd_file, wrap = FALSE)
...
module_help <- function (topic, help_type = getOption("help_type", "text"))
{
  ...
  #    temp = tools::Rd2txt(rd, out = "Rtxt", package = module_name)
  temp=tempfile("Rtxt", fileext = ".Rd")
  writeLines(doc, temp)
  print(temp)
  rstudioapi::previewRd(temp)
  Sys.sleep(1)
  unlink(temp)
}
